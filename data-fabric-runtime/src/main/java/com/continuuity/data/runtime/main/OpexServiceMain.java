...
public class OpexServiceMain {
  ...
  private static void startHealthCheckService(CConfiguration conf) {
    ...
    final CommandPortService service = CommandPortService.builder("opex-status")
      .setPort(port)
      .addCommandHandler("status", "Service status", new CommandPortService.CommandHandler() {
        @Override
        public void handle(BufferedWriter respondWriter) throws IOException {
          respondWriter.write("RUNNING");
          respondWriter.close();
        }
      })
      .build();
