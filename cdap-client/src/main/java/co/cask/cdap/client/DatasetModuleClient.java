...
public class DatasetModuleClient {
   ...
   *
   * @param namespace namespace of the new dataset module
   ...
   * @throws IOException if a network error occurred
   */
  public void add(Id.Namespace namespace, String moduleName, String className, File moduleJarFile)
    throws BadRequestException, AlreadyExistsException, IOException, UnauthorizedException {

    Id.DatasetModule module = Id.DatasetModule.from(namespace, moduleName);
    URL url = config.resolveNamespacedURLV3(namespace, String.format("data/modules/%s", moduleName));
