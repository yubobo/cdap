...
public class StreamHandlerTestRun extends GatewayTestBase {
  ...
  public void testStreamInfo() throws Exception {
    ...
    StreamProperties streamProperties = new StreamProperties("stream_info", 2, formatSpecification, threshold);
  ...
  public void testPutStreamConfigDefaults() throws Exception {
    ...
    FormatSpecification formatSpecification =
      new FormatSpecification(SingleStringRecordFormat.class.getCanonicalName(),
                              null, null);
    StreamProperties streamProperties = new StreamProperties("stream_defaults", 2, formatSpecification, threshold);
    urlConn.getOutputStream().write(GSON.toJson(streamProperties).getBytes(Charsets.UTF_8));
    Assert.assertEquals(HttpResponseStatus.OK.getCode(), urlConn.getResponseCode());
    urlConn.disconnect();
    ...
    StreamProperties expected = new StreamProperties("stream_defaults", 2, (new StreamConfig()).getFormat(), threshold);
  ...
  public void testPutInvalidStreamConfig() throws Exception {
    ...
    urlConn = openURL(String.format("http://%s:%d/v2/streams/stream_badconf/config",
                                    HOSTNAME, port), HttpMethod.PUT);
    urlConn.setDoOutput(true);
    StreamProperties streamProperties = new StreamProperties("stream_badconf", -1, null, threshold);
    urlConn.getOutputStream().write(GSON.toJson(streamProperties).getBytes(Charsets.UTF_8));
    Assert.assertEquals(HttpResponseStatus.BAD_REQUEST.getCode(), urlConn.getResponseCode());
    urlConn.disconnect();
    ...
    streamProperties = new StreamProperties("stream_badconf", 2, formatSpec, threshold);
    urlConn.getOutputStream().write(GSON.toJson(streamProperties).getBytes(Charsets.UTF_8));
    Assert.assertEquals(HttpResponseStatus.BAD_REQUEST.getCode(), urlConn.getResponseCode());
    urlConn.disconnect();
