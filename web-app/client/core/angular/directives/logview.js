...
define(['helpers'], function (helpers) {
  ...
  var Directive = ['$interval', '$timeout', '$compile', 'metricsService', 'dataFactory', 'POLLING_INTERVAL',
    function($interval, $timeout, $compile, metricsService, dataFactory, POLLING_INTERVAL) {
    ...
    var logView = {
      ...
      load: function (entity) {
        ...
        this.set('logMetrics', {
          ...
          INFO: {count: 0, active: false},
          WARN: {count: 0, active: false},
          ERROR: {count: 0, active: false},
          DEBUG: {count: 0, active: false},
          OTHER: {count: 0, active: false}
        });
        this.beforeHTML = jQuery('#logView').html();
        ...
        var logInterval = function () {
          self.resize();
          dataFactory.getFlowLogsNextByAppNameAndId(self.entity.app, self.entity.name,
            'fromOffset='+fromOffset + '&maxSize='+maxSize + '&limit=', function (response) {
              ...
              if (response.length) {
                ...
                jQuery('#logView').append('<div>' + response + '</div>');
              }
        ...
        setTimeout(function () {
          logInterval();
          jQuery('#logView').on('DOMMouseScroll mousewheel', function (event) {
            self.setAutoScroll(event);
          });
        }, 500);

        this.interval = setInterval(logInterval, POLLING_INTERVAL);

      },

      resize: function () {
        jQuery('#logView').css({height: (jQuery(window).height() - 290) + 'px'});
      },
      ...
      logBoxChange: function () {
        var self = this;
        var logViewData = jQuery('#logView').html();
        if (!logViewData) {
          jQuery('#logView').html('');
          return;
        }
        this.afterHTML = jQuery('#logView').html();
      ...
      incrementMetrics: function (type, amount) {
        var count = this.get('logMetrics')[type]['count'] + amount;
        this.get('logMetrics')[type]['count'] = count;
      },
      ...
      processLogEntry: function (record) {
        ...
        var element = jQuery('<code data-log-type="'+entryType+'">' + record.log + '</code>');
      ...
      showLogsByType: function (type) {
        var prevPosition = jQuery('#logView').scrollTop() / jQuery('#logView')[0].scrollHeight;
        this.set('logType', type);
        this.setActiveTab(type);
        var self = this;
        var entries = jQuery('#logView code');
        if (type !== 'ALL') {
          for (var i = 0; i < entries.length; i++) {
            if (jQuery(entries[i]).attr('data-log-type') !== type) {
              jQuery(entries[i]).hide();
            } else {
              jQuery(entries[i]).show();
            }
          }
        } else {
          jQuery('#logView code').show();
        }
        jQuery('#logView').scrollTop(jQuery('#logView')[0].scrollHeight * prevPosition);
      },
      ...
      setActiveTab: function (type) {
        ...
        for (var metric in logMetrics) {
          if (logMetrics.hasOwnProperty(metric)) {
            logMetrics[metric]['active'] =  false;
          }
        }
        logMetrics[type]['active'] = true;
      },
      ...
      logUp: function () {
        ...
        dataFactory.getFlowLogsPrevByAppNameAndId(
          self.entity.app, self.entity.name,
          'fromOffset='+initialOffset + '&maxSize='+maxSize + '&limit=', function (response) {
            ...
            if (response.length) {
              ...
              if (response.length) {
                var currentScroll = jQuery('#logView').scrollTop();
                jQuery('#logView').prepend('<div>' + response + '</div>');
                var responseHeight = jQuery('#logView div:first')[0].scrollHeight;
                jQuery('#logView').scrollTop(responseHeight + currentScroll);
              }
            }
      ...
      scrollLogDown: function() {
        jQuery('#logView').scrollTop(
          jQuery('#logView')[0].scrollHeight - jQuery('#logView').height());
      },
    ...
    return {
      ...
      restrict: 'AE',
      templateUrl: '/templates/directives/logview.html',
      scope: {
        data: '='
      },
      link: function (scope, elm, attrs) {
        scope.logView = logView;
        scope.$watch('data', function (newVal, oldVal) {
          if (angular.isObject(newVal) && Object.keys(newVal).length) {
            scope.logView.load(scope.data);
          }
        });

        scope.showLogsByType = function (type) {
          logView.showLogsByType(type);
        }
